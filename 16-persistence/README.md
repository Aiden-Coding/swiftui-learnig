...
1. 在工程中创建本章练习视图：Chapter16PracticeView。
...
struct Chapter16PracticeView: View {
    @State private var counter = 0
    @State private var showDetail = false

    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            Text("第 16 章：数据持久化入门：AppStorage")
                .font(.headline)

            Text("核心目标：保存并恢复轻量用户配置。")
            ...
            if showDetail {
                Text("练习提示：UserDefaults 适合轻量 key-value，不适合复杂数据。")
                    .font(.footnote)
                    .padding(10)
...
UserDefaults 适合轻量 key-value，不适合复杂数据。

## 本章小结

你已经完成第 16 章的核心学习。建议花 10 分钟做一次“复述训练”：不用看文档，口头讲清本章的概念、代码结构和排错方法，这会显著提升你的掌握度。

...
- 关键词：NavigationStack、NavigationLink、navigationTitle
- 一句话理解：完成列表到详情页的基础导航。
...
struct Chapter07PracticeView: View {
                ...
                if showDetail {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("学习提示")
                            .font(.subheadline)
                            .fontWeight(.semibold)
                        Text("现代 SwiftUI 优先使用 NavigationStack。")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
                    .padding(12)
...
现代 SwiftUI 优先使用 NavigationStack。

## 本章小结

本章结束后，你应该已经能完成：搭建两页以上流程。。如果还不稳，先重复“手把手步骤”一次，再进入下一章。
...
希望 Siri 或快捷指令能触发应用动作。

## 本章术语先看懂

- 关键词：AppIntent、Parameter、IntentResult
- 一句话理解：把应用能力暴露给系统自动化。

## 手把手步骤（每一步都有预期结果）

1. 创建视图 Chapter35PracticeView。  
...
struct Chapter35PracticeView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                Text("第 35 章：App Intents 入门")
                    .font(.headline)

                Text("核心目标：把应用能力暴露给系统自动化。")
...
本章结束后，你应该已经能完成：实现系统入口调用动作。。如果还不稳，先重复“手把手步骤”一次，再进入下一章。
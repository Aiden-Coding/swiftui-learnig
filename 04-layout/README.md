...
- 关键词：Stack、alignment、spacing、Spacer
- 一句话理解：掌握三大布局容器与 Spacer 的作用。

## 手把手步骤（每一步都有预期结果）

1. 创建视图 Chapter04PracticeView。  
...
struct Chapter04PracticeView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                Text("第 04 章：布局系统：VStack、HStack、ZStack、Spacer")
                    .font(.headline)

                Text("核心目标：掌握三大布局容器与 Spacer 的作用。")
...
先定层级，再调间距，不要先乱加 padding。

## 本章小结

本章结束后，你应该已经能完成：能根据目标结构选容器。。如果还不稳，先重复“手把手步骤”一次，再进入下一章。
...
- 关键词：Text、Image、Button
- 一句话理解：会组合 Text、Image、Button 构建基础界面。

## 手把手步骤（每一步都有预期结果）

1. 创建视图 Chapter03PracticeView。  
                ...
                if showDetail {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("学习提示")
                            .font(.subheadline)
                            .fontWeight(.semibold)
                        Text("优先使用 SF Symbols，能减少素材准备成本。")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
                    .padding(12)
...
本章结束后，你应该已经能完成：组合出可点击的模块卡片。。如果还不稳，先重复“手把手步骤”一次，再进入下一章。
...
- 关键词：body、some View、modifier
- 一句话理解：掌握 View、body、修饰符三者关系。

## 手把手步骤（每一步都有预期结果）

1. 创建视图 Chapter02PracticeView。  
                ...
                if showDetail {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("学习提示")
                            .font(.subheadline)
                            .fontWeight(.semibold)
                        Text("遇到 some View 不要慌，先把它当作“某种视图”。")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
                    .padding(12)
...
本章结束后，你应该已经能完成：独立写出一个带样式标题。。如果还不稳，先重复“手把手步骤”一次，再进入下一章。